...
class Neo4jConnection:
    ...
    def create_indexes(self):
        with self.driver.session() as session:
            ...
            session.run("CREATE INDEX IF NOT EXISTS FOR (a:Agent) ON (a.AGENT)")
            session.run("CREATE INDEX IF NOT EXISTS FOR (p:Agent) ON (p.PICK_RATE)")
            ...
            with open(csv_file, 'r', encoding='utf-8') as file:
                ...
                for row in reader:
                    ...
                    session.run(
                        ...
                        parameters={'AGENT': row['AGENT'], 'PICK_RATE': row['PICK_RATE'], 'ACS': float(row['ACS'])}
                    )
